...
int main(int argc, char* argv[]) {
    ...
    while (is_playing) {
        ...
        // --- OPCODE 0xF3: HIGH-SPEED CONSTANT AUDIO STREAM ---
        if (opcode == OP_AUDIO_START) {
            // Instantly grab exactly 1102 bytes from the stream
            fread(chunk_buffer, 1, FIXED_AUDIO_SIZE, f);
            
            // Queue it directly into the hardware engine
            SDL_QueueAudio(audio_device, chunk_buffer, FIXED_AUDIO_SIZE);

            // Swallow the trailing 0xF4 marker to cleanly cycle to the next block
            uint8_t audio_end;
            fread(&audio_end, 1, 1, f);
        }
        // --- OPCODE 0xF1: DELTA VIDEO FRAMES ---
        else if (opcode == OP_FRAME_START) {
